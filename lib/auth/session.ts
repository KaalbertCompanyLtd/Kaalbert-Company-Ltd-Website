 ...
 * window — and, on success, bumps `lastActivityAt` to now (this call *is* the "activity"
 * that resets the inactivity clock). A session that fails either check here is deleted
 * outright, not merely ignored, *once its own token is actually presented* — this is lazy
 * expiry, not a sweep: a different session row that's gone equally stale but whose cookie is
 * never presented again (e.g. an abandoned browser tab) simply sits in `admin_session`
 * indefinitely, functionally inert (it would fail this same check if it ever were presented)
 * but not proactively deleted. No cleanup job exists for this table yet, same "proportionate
 * for now, revisit if real volume changes" call as `AdminLoginAttempt`'s own doc-comment
 * makes for the same reason (five partners, occasional logins) — confirmed for real at T6.3
 * (Playwright verification, session 39): backdating a session's `lastActivityAt` and then
 * presenting a *different* session's still-valid token left the backdated row untouched in
 * the table, exactly as this lazy-expiry design predicts. Returns `null` for no session, not
 * found, or either expiry reason; the caller (`proxy.ts`, or a route handler) never needs to
 * distinguish which.
 */
export async function verifySession(token: string): Promise<VerifiedSession | null> {
  ...
  if (session.expiresAt < now || inactiveForMs > INACTIVITY_TIMEOUT_MS) {
