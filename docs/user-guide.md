...
still need it to actually log in afterward. Once you're already signed in, you don't need
this flow at all to change your password — see "Account & security" below.
...
backup codes, there is no self-service recovery — ask an Owner to reset your 2FA enrolment
from your entry under `/admin/team` (see "Team" below). Voluntarily switching to a new phone
or authenticator app while you still have the old one is different and _is_ self-service —
see "Account & security" below.

**Getting your very first account** is now handled inside the admin area itself, not by the
developer: an Owner invites you from `/admin/team` (see "Inviting a new partner" under
"Editing your team profile" below). You'll receive an email with a temporary password and a
link to set up your authenticator app — both are needed the first time you sign in.

**Signing out.** Click your name at the bottom of the sidebar (or inside the menu, on mobile)
to open the account menu, then **Sign out**. This ends your session immediately, on this
device only.

### Account & security — `/admin/account`

Reached from the account menu (click your name in the sidebar, then **"Account & security"**).
Manages only your own login — nothing here affects anyone else's account.

- **Identity**: your name, email, and role (Owner or Partner) — read-only. If your email
  needs to change, ask an Owner (or, if you are the only Owner, the developer) to update it
  directly.
- **Change password**: enter your **current password**, then a **new password** (12+
  characters) and **confirm new password**, click **Change password**. This signs you out of
  every device, including this one — sign back in with the new password right after.
- **Set up a new device**: click this if you're switching phones or authenticator apps and
  still have your old one working. It takes you straight to the same QR-code setup screen
  used for a brand-new account.
- **Regenerate backup codes**: click this if you've used most of your 8 backup codes, or
  think they may have been seen by someone else. Your old, unused codes stop working the
  moment the new 8 are shown — write the new set down somewhere safe, since they're shown
  only once.
...
`/admin/enquiries` — click **Enquiries** in the sidebar, under Operations.

**What it shows:** every enquiry ever submitted (diagnostic or contact form), one row per
...
`/admin/team` — click **Team** in the sidebar for the list of all partners (name, title,
practice area, whether published on `/about`, whether they have a login yet and its status,
and their **Role** — Owner or Partner — with "(you)" marking your own row). Click any name to
open that entry.

**Who can edit what:** you can always edit your own entry. You can open anyone else's to view
it, but only an **Owner** can actually edit someone else's profile or act on their login — a
Partner viewing a colleague's entry sees every field, read-only, with a note explaining why.
...
4. Set **Display order** — the lowest number becomes the single featured "Lead Partner" card
   on `/about`.
5. Use the **Published / Not published** switch near the top to control whether this profile
   is live on `/about` and in article bylines.
6. Click **Save**.

**Publishing is a real switch you control, not something the system decides for you.** You
can only turn it **on** once Name, Practice area, and Personal statement are all filled in —
the switch is disabled (with an explanation) until then. Once those are filled, you choose
whether the profile is published or not, and you can turn it back **off** at any time — even
for a partner who already has published Insights articles crediting them (those articles then
credit "Kaalbert & Company Ltd" instead of a byline, so nothing breaks). Photo and credentials
are never required for publishing.

### Inviting a new partner (Owner only)

Click **"Add partner"** on the Team list (only visible to Owners) to open `/admin/team/new`.

1. Choose a mode at the top: **"Link an existing profile"** (for one of the partner profiles
   already in the system that has no login yet) or **"Create a brand-new partner"** (for
   someone with no profile at all yet).
2. **Linking an existing profile**: pick their name from the **Partner profile** dropdown —
   their existing photo/name/title show below so you can confirm it's the right person.
3. **Creating a brand-new partner**: fill in the same fields as "Editing your team profile"
   above (photo, Name, Title, Practice area, Credentials, Personal statement, Bio) — their
   profile starts unpublished; publish it from their entry once it's complete.
...
appears below the editor on their entry (Owners only — a Partner viewing their own entry
manages their own login from **Account & security** instead, and doesn't see this panel at
all when viewing someone else's), showing their email and Active/Deactivated status:

- **Role** — a dropdown to promote or demote between Owner and Partner. The firm can never be
  left with zero Owners — demoting the last one is blocked.
- **Deactivate account** — click it, confirm in the dialog. Ends every session that partner
  ...
  affects their login. You can't deactivate your own account this way — ask another Owner.
...
- **Reset 2FA enrolment** — click it. A fresh re-enrolment link appears on screen (shown once
  — relay it to the partner directly through whatever secure channel you'd use). Use this
  when a partner has lost both their device and their backup codes.
- **Reset password** — click it. Same pattern: a one-time reset link appears on screen, relay
  it to the partner directly.

**What to monitor:** inviting the 5 real partners (linking their real email addresses to
their existing profiles) is a firm decision, not something done automatically by this
system — each one still shows "No login yet" until an Owner sends them a real invite. No real
partner photography exists yet either — every profile showing an initials avatar is the
correct, intended state right now, not a bug.
...
- Email delivery for the diagnostic's summary-email step depends on the **Brevo account**,
  now sending as `info@kaalbert.com` (switched 2026-09-16, verified working end-to-end) — if
  summary emails stop arriving, check Brevo's sender-verification status first.
...
Once the original content is filled in, this is the complete picture of what actually
becomes recurring partner work — today (Phase 1, live) and everything Phase 2 adds once each
...
| Date       | What changed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-12 | Added an **"Ongoing work, at a glance"** table — a single, complete reference for what recurring partner work actually looks like once original content is filled in: today's live Phase 1 work (enquiries, articles, landing pages) plus every Phase 2 capability's own ongoing workload once its trigger is met and it's actually built, so the firm can see the whole picture — not built yet, not scheduled — in one place instead of piecing it together from `docs/scope.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
